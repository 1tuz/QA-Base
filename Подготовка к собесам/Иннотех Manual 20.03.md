...
Например:
2 QA работаю одновременно над одним проектом, но на разных ветках. В конце их надо объединить. Это делается с помощью гита, безопасно, чтобы ни у кого ничего не удалилось\заменилось. 

**Пример:**
Приходит второй чел, делается ответвление. Каждый вносит свои правки и обновления. Когда приходит время объединять, то версия, на которой мы разветвились берется за главную. Затем каждый отдает свою версию на главную версию. Если файлы не сходятся (по коду) то он запросто объединит, оставив все. Если есть одинаковые строки, то он просто спросит, чью строку оставить. 
Если все хуйня, то просто можно откатиться на изначальную или последнюю актуальную версию и переделываем. 
