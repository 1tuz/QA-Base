...
**`GROUP BY`** – группирует строки с одинаковыми значениями, используется с агрегатными функциями. Например, если у нас есть таблица платежей, и мы хотим узнать, сколько денег потратил каждый пользователь, мы группируем данные по `user_id` и считаем сумму **`SUM(column).
